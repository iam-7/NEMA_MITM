...
config_data = json.load(open('attr_config.json', 'r'))

targets = list(config_data["attributes"])
print(targets)
for data in data_set:
    #print(data)
    sentence_type = data.split(",")[0][3:]
    attr_to_modify = list()
    for attr in targets:
    ...
    if len(attr_to_modify) > 0:
        ...
        #data = str(nmea_obj.sentence)
        print(str(nmea_obj.sentence))
        print()
